...
public class DirectoryChooserDialog
{
    ...
    public void chooseDirectory(String dir)
    {
        ...
        class DirectoryOnClickListener implements DialogInterface.OnClickListener
        {
            ...
            {
                if(!m_sdcardDirectory.equals(m_dir) && item == 0){
                    // Navigate into the sub-directory
                    String currentDirectory = m_titleView.getText().toString();
                    String[] arr = currentDirectory.split("/");
                    String parentDirectory = "";
                    for (int i = 0; i < arr.length; i++) {
                        if (i != 0 && i != arr.length - 1) {
                            parentDirectory = parentDirectory + "/" + arr[i];
                        }

                    }
                    m_dir = parentDirectory;
                    updateDirectory();
                }else{
                    // Navigate into the sub-directory
    ...
    private List<String> getDirectories(String dir)
    {
        ...
        if (!m_sdcardDirectory.equals(dir)) {
            dirs.add(0, "返回上一级");
        }
            ...
            DialogInterface.OnClickListener onClickListener)
    {
        ...
        m_titleView.setLayoutParams(new LayoutParams(Common.convertDpToPixels(200, m_context), LayoutParams.WRAP_CONTENT));
        ...
        m_titleView.setEllipsize(TextUtils.TruncateAt.MARQUEE);
        m_titleView.setSingleLine(true);
        m_titleView.setMarqueeRepeatLimit(100);
        m_titleView.setFocusable(true);
        m_titleView.setFocusableInTouchMode(true);
                ...
                new AlertDialog.Builder(m_context).
                        ...
                        setView(input).setPositiveButton("确定", new DialogInterface.OnClickListener()
                        {
                            public void onClick(DialogInterface dialog, int whichButton)
                            {
                                ...
                                {
                                    Toast.makeText(
                                            m_context, "创建'" + newDirName +
                                                    "' 文件夹失败", Toast.LENGTH_SHORT).show();
                                }
                            }
